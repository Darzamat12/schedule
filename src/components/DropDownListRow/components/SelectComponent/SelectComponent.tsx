...
const SelectComponent:React.FC<{ initialValue: number,
  optionData: { name: string; value: number; }[], changeFunction: Function}> = (props) => {
    ...
    <>
      <Select defaultValue={props.initialValue} style={{ width: 200 }} onChange={handleChange} disabled={false}>
        { props.optionData.map((item, i) => {
            return <Option key={item.name} value={i}>{item.name}</Option>
        })}
      </Select>
    </>
  );
